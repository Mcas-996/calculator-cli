...
pub fn solve_quadratic(coeffs: &[ComplexNumber]) -> Result<Vec<ComplexNumber>, String> {
    ...
    let discriminant = b.clone() * b.clone() - ComplexNumber::from_real(Fraction::new(4, 1)) * a.clone() * c.clone();
    let sqrt_discriminant = discriminant.sqrt();

    let x1 = (-b.clone() + sqrt_discriminant.clone()) / (ComplexNumber::from_real(Fraction::new(2, 1)) * a.clone());
    let x2 = (-b.clone() - sqrt_discriminant) / (ComplexNumber::from_real(Fraction::new(2, 1)) * a.clone());
...
fn parse_quadratic_coefficients(equation: &str) -> Result<Vec<ComplexNumber>, String> {
    // Simplified parsing: look for pattern like "ax² + bx + c = 0"
    let eq_lower = equation.to_lowercase();
    let eq = eq_lower.replace(" ", "").replace("²", "^2");
