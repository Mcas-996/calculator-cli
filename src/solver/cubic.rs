...
pub fn solve_cubic(coeffs: &[ComplexNumber]) -> Result<Vec<ComplexNumber>, String> {
    ...
    let q = (2.0 * b_val * b_val * b_val - 9.0 * a_val * b_val * c_val + 27.0 * a_val * a_val * d_val) 
    ...
    if discriminant.abs() < 1e-10 {
        ...
        roots.push(ComplexNumber::from_doubles(real_part - b_val / (3.0 * a_val), imag_part));
        roots.push(ComplexNumber::from_doubles(real_part - b_val / (3.0 * a_val), -imag_part));
    } else {
        // Three real roots
...
fn parse_cubic_coefficients(equation: &str) -> Result<Vec<ComplexNumber>, String> {
    ...
    let eq = eq_lower.replace(" ", "").replace("³", "^3").replace("²", "^2");
