...
pub fn solve_linear_system(equations: &[String]) -> Result<Vec<(String, ComplexNumber)>, String> {
    ...
    let vars: Vec<String> = (0..n).map(|i| format!("x{}", i + 1)).collect();

    Ok(vars.into_iter().zip(solutions).collect())
}

/// Parse system equations into augmented matrix
fn parse_system_equations(
    equations: &[String],
    n: usize,
) -> Result<Vec<Vec<ComplexNumber>>, String> {
...
mod tests {
    ...
    fn test_solve_2x2_system() {
        let equations = vec!["x + y = 5".to_string(), "x - y = 1".to_string()];
